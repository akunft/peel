...
class FlinkFastRExperiment(
  ...
  override def run(id: Int, force: Boolean): Experiment.Run[Flink] = {
    new FlinkFastRExperiment.SingleJobRun(id, this, force)
  }

  def copy(name: String = name, config: Config = config) = {
    new FlinkFastRExperiment(command, systems, runner, runs, inputs, outputs, name, config)
  }
}

object FlinkFastRExperiment {
  ...
  class SingleJobRun(val id: Int, val exp: FlinkFastRExperiment, val force: Boolean) extends Experiment.SingleJobRun[Flink, State] {

    import org.peelframework.flink.beans.experiment.FlinkFastRExperiment.StateProtocol._
...
//      state.plnExitCode = Some(plnExit)
      ...
      val (runExit, t) = Experiment.time(this !(s"${command.trim}", s"$home/run.out", s"$home/run.err"))
      state.runTime = t
